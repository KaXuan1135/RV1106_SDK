...
inline static const char* get_error_message(int error_code)
{
    static char buffer[256];

    switch(error_code) {
    case RKNN_SUCC: 
        std::sprintf(buffer, "[%d] Success", error_code);
        break;
    case RKNN_ERR_FAIL: 
        std::sprintf(buffer, "[%d] Execution failed", error_code);
        break;
    case RKNN_ERR_TIMEOUT: 
        std::sprintf(buffer, "[%d] Execution timeout", error_code);
        break;
    case RKNN_ERR_DEVICE_UNAVAILABLE: 
        std::sprintf(buffer, "[%d] Device is unavailable", error_code);
        break;
    case RKNN_ERR_MALLOC_FAIL: 
        std::sprintf(buffer, "[%d] Memory allocation failed", error_code);
        break;
    case RKNN_ERR_PARAM_INVALID: 
        std::sprintf(buffer, "[%d] Invalid parameter", error_code);
        break;
    case RKNN_ERR_MODEL_INVALID: 
        std::sprintf(buffer, "[%d] Invalid model", error_code);
        break;
    case RKNN_ERR_CTX_INVALID: 
        std::sprintf(buffer, "[%d] Invalid context", error_code);
        break;
    case RKNN_ERR_INPUT_INVALID: 
        std::sprintf(buffer, "[%d] Invalid input", error_code);
        break;
    case RKNN_ERR_OUTPUT_INVALID: 
        std::sprintf(buffer, "[%d] Invalid output", error_code);
        break;
    case RKNN_ERR_DEVICE_UNMATCH: 
        std::sprintf(buffer, "[%d] Device unmatched, please update RKNN SDK and NPU driver/firmware", error_code);
        break;
    case RKNN_ERR_INCOMPATILE_PRE_COMPILE_MODEL: 
        std::sprintf(buffer, "[%d] The pre-compiled RKNN model is incompatible with the current platform", error_code);
        break;
    case RKNN_ERR_INCOMPATILE_OPTIMIZATION_LEVEL_VERSION: 
        std::sprintf(buffer, "[%d] The optimization level of RKNN model is incompatible with the current driver", error_code);
        break;
    case RKNN_ERR_TARGET_PLATFORM_UNMATCH: 
        std::sprintf(buffer, "[%d] The specified target platform is incompatible with the current platform", error_code);
        break;
    default: 
        std::sprintf(buffer, "[%d] UNKNOWN", error_code);
        break;
    }
...
inline static const char* get_core_mask_string(rknn_core_mask core_mask)
{
    switch(core_mask) {
    case RKNN_NPU_CORE_AUTO: return "RKNN_NPU_CORE_AUTO";
    case RKNN_NPU_CORE_0: return "RKNN_NPU_CORE_0";
    case RKNN_NPU_CORE_1: return "RKNN_NPU_CORE_1";
    case RKNN_NPU_CORE_2: return "RKNN_NPU_CORE_2";
    case RKNN_NPU_CORE_0_1: return "RKNN_NPU_CORE_0_1";
    case RKNN_NPU_CORE_0_1_2: return "RKNN_NPU_CORE_0_1_2";
    case RKNN_NPU_CORE_ALL: return "RKNN_NPU_CORE_ALL";
    case RKNN_NPU_CORE_UNDEFINED: return "RKNN_NPU_CORE_UNDEFINED";
