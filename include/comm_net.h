...
#define MSG_TYPE_UPDATE_LOCAL_INFO	0xA00D	//搜索用的
...
#define	MSG_TYPE_REBOOT_LOADER				0xA025	//进入烧录模式
#define	MSG_TYPE_REBOOT_RECOVERY			0xA026	//进入tf卡 烧录模式
#define MSG_TYPE_SET_DEFAULT_PARAM			0xA027	//设置默认参数
...
#define	ISP_TYPE_ROT_CONFIG			0x09
...
struct __local_info_t__
{
	...
	char serial_info[MAX_COMM_SERIAL_LEN];
	unsigned char status_4G:2;	// 4G 状态	0x00: 设备不存在      0x01: 无 sim 卡 0x02 已经拨号上 0x03 其他故障
	unsigned char status_wifi:2;	// wifi 状态 0x00: 设备不存在 , 0x01:用户或密码不正确 0x02 已经 0x03 其他故障
	unsigned char status_sd:2;	// sd 卡状态，0x00 tf 卡不存在 
	unsigned char status_res:2;	//保留
	unsigned char signal;		//如果是wifi的话，是wifi的信号，如果是4G 的话，是4G的信号
	unsigned short version;
	unsigned int  run_time;				//程序运行时间,按秒计算
	int  device_type;		// 
	char auth_flag;	 		//
	char led_type;			//暂时没用
	char cam_count;			//摄像机个数
	char tempture;			//cpu 温度
