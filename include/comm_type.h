...
enum app_exposure_mode {APP_AUTO_EXP_MODE = 0,APP_MANUAL_EXP_MODE = 1, APP_SPEC_EXP_MODE=2};
enum app_ircut_mode { APP_IRCUT_MODE_MANUAL = 0, APP_IRCUT_MODE_AUTO };
enum app_daynight_mode {APP_DAYNIGHT_AUTO_MODE = 0,APP_DAYNIGHT_DAY_MODE =1,APP_DAYNIGHT_NIGHT_MODE = 2,APP_DAYNIGHT_SPEC_MODE=3};
enum app_led_mode {APP_LED_IR_MODE = 0,APP_LED_WHITE_MODE = 1,APP_LED_DAUL_MODE = 2,APP_LED_NONE = 3};
enum app_hdr_mode	{APP_HDR_NORMAL_MODE = 0, APP_HDR_HDR2_MODE = 1,APP_HDR_HDR3_MODE =2};
enum app_blc_mode {APP_BLC_CLOSE_MODE = 0, APP_BLC_OPEN_MODE = 1};
enum app_hlc_mode {APP_HLC_CLOSE_MODE = 0, APP_HLC_OPEN_MODE = 1};
enum app_wb_mode {APP_WB_AUTO_MODE = 0, APP_WB_MANUAL_GAIN_MODE = 1, APP_WB_MANUAL_SCENE_MODE = 2, APP_WB_MANUAL_CT_MODE = 3,};
enum app_nr_mode {APP_NR_DEFAULT_MODE = 0,APP_NR_TF_MODE = 1,APP_NR_SF_MODE = 2,APP_NR_TSF_MODE=3};
enum app_forg_mode {APP_FORG_CLOSE_MODE = 0,APP_FORG_OPEN_MODE = 1};
enum app_abc_mode {APP_ABC_LDHC_MODE = 0, APP_ABC_FEC_MODE = 1,APP_ABC_CLOSE_MODE = 2};
enum app_mirror_mode {APP_MIRROR_NORMAL_MODE = 0, APP_MIRROR_LR_MODE = 1,APP_MIRROR_UD_MODE = 2,APP_MIRROR_R90_MODE = 3,APP_MIRROR_R270_MODE = 3};
...
#define RECORD_FILE_TYPE_RAW	0x00
#define	RECORD_FILE_TYPE_FLV	0x01
#define	RECORD_FILE_TYPE_MP4	0x02
#define MAX_RECORD_FILE_NUM		9999
...
#define MAX_POLYGON_LINE_NUM 10
typedef struct avcodec_osd_polygon_s
{
	int index;	//range: 0 -7 
	int enable;
	int num_line;	
	short coordinates[MAX_POLYGON_LINE_NUM * 2];
	int thick;
	unsigned int color;
	
}avcodec_osd_polygon_t;
...
typedef struct avcodec_osd_str_s
{
	int index;	//range: 0 -30
	int enable;
	unsigned int x;			
	...
	unsigned int color;  // rr gg bb , rr is  at higher bit.
...
typedef int (*net_stream_cb_fun_t)(int ch,int sub_ch,void *pFrame,void *user_data);
typedef int (*nn_stream_cb_fun_t)(int ch,void *pFrame,void *nn_obj,void *user_data);

typedef struct __record_file_info_t
{
	int ch;
	int ch_type;
	int nIndex;
	int start_time;	//开始时�?
	int duration;	//录像时长,sec
	int file_type;	//录像文件类型
	char szFileName[MAX_PATH_SIZE];
	int status;
}record_file_info_t;

typedef struct __record_file_mgr_t
{
	int nCurr;
	int nBase;
	int nIndex;
	...
	record_file_info_t file_info[MAX_RECORD_FILE_NUM];
}record_file_mgr_t;
