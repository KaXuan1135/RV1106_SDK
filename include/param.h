...
#define DEFAULT_SERIALP_BAUD 9600
...
{
	char	sample_rate;	/* 采样率 0:8k, 1:16K */
	char	bit_width;	/* 位宽 0: 8bit, 1: 16bit */
	char	encode_type;	/* 编码方式 0:pcm, 1:g711a, 2:g711u, 3:aac */
	char	amr_mode;  /*  unused bit rate type 0:4.75k,1:5.15k,2:5.90K,3:6.70K,4:7.40k,5:7.95k,6:10.20k,7:12.20k */
	char	amr_format; /* unused package type 0:MIME(file saving), 1:IF1(wireless), 2:IF2(wireless) */
	
	char	volume_microphone;
	char	volume_speaker;
	char	unused[5];
}comm_audio_encode_t;
...
typedef struct __comm_network_config_t
{
	...
	uchar_t 	dhcp_flag2;	/* wifi dhcp 使能标志,0: 禁止, 1, 使能 */
...
typedef struct {
	char timezone[40];
	uchar_t is_summer_time;
	uchar_t is_plat_adjust;
}comm_real_timer_t;
...
//for web rtc
typedef struct {
	uchar_t web_language; // 0 --- english, 1--- chinese,
	uchar_t partchn ; // 级联 是否打开
	uchar_t are_auto_reboot; // 是否自动重启
	uchar_t rebootDay;      //自动重启间隔
	uchar_t rebootHour;
	uchar_t rebootMin;
	uchar_t rebootSec;
...
typedef struct
{
	uchar_t 	enable; /* 是否使能无线网络  1: 使能	0:禁用 */
	char ssid[24];
	char passwd[16];
	char enc_mode[16]; //加密模式，这个一般取决于 AP
...
}comm_wireless_config_webrtc;

typedef struct 
{
	uchar_t enable; 
	char imei[20];
	uchar_t signal_quality; // 信号质量
	unsigned int flow; 		// 流量
		
}common_4G_config_webrtc;

typedef struct 
{
	uchar_t open_enable; //0：关闭 1: 打开
	char opserver_ip[COMM_MAX_ADDR_SIZE];
	ushort_t opport;
	char op_uuid[20];
	char password[20];
	uint16_t optimeout;
...
}common_platform_config_webrtc;

#define MAX_ALARM_RCTRL_NUM 16
typedef struct {
	//(1：学习编码； 2：配件扫描；3：删除；4：删除全部)
	uchar_t operate;
	/*(1：普通防区； 2：内部防区；3：漏水；4：燃气；5：烟感； 6：救护；7：劫持；
	 8：有声紧急； 9：出入防区；10：防拆；11：智能；12：门铃)
	*/
	uchar_t sectorattr;
	uchar_t status;		//(0：停用; 1：启用)
	char sectorid[4];	
}common_alarm_rctrl_config_webrtc;

#define MAX_ALARM_WIRELESSDA_NUM 32
typedef struct {
	//(1：学习编码； 2：配件扫描；3：删除；4：删除全部)
	uchar_t operate;
	/*(1：普通防区； 2：内部防区；3：漏水；4：燃气；5：烟感； 6：救护；7：劫持；
	...
	*/
	...
	char sectorid[4];	
	char voice[30];	
}common_alarm_wirelessda_config_webrtc;

#define MAX_ALARM_WIREDDA_NUM 4
typedef struct {
	/*(1：普通防区； 2：内部防区；3：漏水；4：燃气；5：烟感； 6：救护；7：劫持；
	 8：有声紧急； 9：出入防区；10：防拆；11：智能；12：门铃)
	*/
	...
	char sectorid[4];	
	char voice[20];	
}common_alarm_wiredda_config_webrtc;

#define MAX_ALARM_AUTO_NUM 4
typedef struct {
	uchar_t auto_week_day[7];
	
	uchar_t auto_event;
	char auto_time[8];

}common_alarm_auto_config_webrtc;

typedef struct {
	uchar_t auto_week_day[7];
	
	uchar_t pgm1;  //(1：布防；2：报警；3：VOIP)
	uchar_t pgm2;
	uchar_t relay1;
	uchar_t relay2;
	uchar_t light;
...
}common_alarm_sndout_config_webrtc;

typedef struct {
	uchar_t wel_mode; //(0：关闭；1：开启)
	uchar_t wel_voice_id; //(1：欢迎光临；2：自定义)
	char wel_voice_text[20];
	uchar_t wel_time;
	uchar_t voice_id;//(1：警号；2：发生报警，请注意；3：自定义)
	char voice_text[30];
	uchar_t drivevoice_id; //(1：你已进入警戒区域，请迅速离开（默认）；2：你已进入危险区域，请勿靠近；3：你已进入私人区域，禁止入内；4：水深危险，注意安全；5：高处危险，请勿攀爬；6：自定义)
	char drivevoice_text[30];

}common_alarm_voice_config_webrtc;

typedef struct {
	uchar_t upload;
	uchar_t pretime;
	uint16_t sustaindtime;
	uint16_t interval;
	char other_fq[24];
}common_alarm_video_config_webrtc;
...
typedef struct {
	...
	uint16_t interval;
	char other_fq[25];
}common_alarm_image_config_webrtc;

typedef struct {
	uchar_t shieldmsg;//(0：不屏蔽；1：屏蔽)
	uchar_t interval; //(1-12小时)
}common_alarm_other_config_webrtc;

#define MAX_ALARMSET_NUMBER 4
typedef struct {
	uchar_t status;//(0：停用; 1：启用)
	char name[16]; //通道名
	uchar_t mode;
	char checktype[8]; //(1：人形；2：车辆；3：非机动车；)
	uchar_t dirvemode;
	uchar_t number;
	uchar_t time;
	uchar_t dirveset;
}common_analyze_alarmset_config_webrtc;

typedef struct {
	uchar_t sen; //(1：高; 2：中；3：低)
	uchar_t rule;
	uchar_t shape;
	uchar_t track;
	uchar_t alarm_interval;
	uchar_t fault;
	uchar_t rule_box;
	uchar_t move_auto;
	short rule_coordinate[16]; //(规则框坐标点，百分比 * 100), 里面的第一个值超过10000 表示空。
	short occ1_coordinate[16]; //遮挡区域
	short occ2_coordinate[16];
	short occ3_coordinate[16];
	short occ4_coordinate[16];
	short sec1_coordinate[16];
	short sec2_coordinate[16];
	short sec3_coordinate[16];
	short sec4_coordinate[16];
	uchar_t bright;
	uchar_t contrast;
	uchar_t saturation;
	uchar_t colour;

}common_analyze_region_config_webrtc;

// partition
typedef struct {
	char mode[12];
	char protocol[8];
}common_partition_mode_config_webrtc;

/*
typedef struct {

}common_partition_gbset_config_webrtc;
 // 使用 gb_platform_t 来代替。
****/
...
}common_partition_voipaccount_config_webrtc;


typedef struct {
	uchar_t type_1;
	char daynumber1_1[16];
	char daynumber2_1[16];
	char daynumber3_1[16];
	char nightnumber_1[16];
	uint16_t circulate_1;
	
	uchar_t type_2;
	char daynumber1_2[16];
	char daynumber2_2[16];
	char daynumber3_2[16];
	char nightnumber_2[16];
	uint16_t circulate_2;
	
	char daystarttime[8];
	char nightstarttime[8];
	char autotime[8];
	uchar_t talk_mode;
}common_partition_vkey_config_webrtc;
...
#define MAX_PC_RESOLUTION_MAX_SIZE 1000
typedef enum{ 
...
struct server_config_s
{
	...
	int ptz_enable;	//是否开启ptz 发送af 数据
	int rtsp_enable;
	int onvif_enable;
	int gb28181_enable;
	int gat1400_enable;
	...
	int wifi_enable;
	int wifi_mode;
	char wifi_ssid[32];		//为sta模式下的ssid
	char wifi_passwd[32];	//为 sta 模式下的passwd
	char wifi_ap_ssid[32];	//ap 模式下的ssid
	char wifi_ap_passwd[32]; //ap 模式下的passwd
	int key_len;
	int key[32];
	char uuid[32];
...
#define MAX_VOIP_DIAL_NUM	2
typedef  struct voip_dial_s voip_dial_t;
...
struct voip_dial_s
{
	...
	int key_attr;
	int interval_time;	//循环拨号间隔

	char local_dial_name[32];
	char local_dial_disp_name[32];
	char local_dial_userId[32];
	char local_dial_passwd[32];
	
	///	每一个dial 有两个注册服务器
	char server1_addr[64];
	int server1_port;
	int server1_transport;
	int server1_expire_time;
	int server1_keeplive_time;

	char server2_addr[64];
	int server2_port;
	int server2_transport;
	int server2_expire_time;
	int server2_keeplive_time;
	
	char dail0[32];
	char dial1[32];
	char dial2[32];
	char dial3[32];	//晚上拨号时间	
	comm_time_seg_t start_time;
	comm_time_seg_t stop_time;

};

typedef struct voip_config_s voip_config_t;
struct voip_config_s
{
	unsigned int version;
	voip_dial_t dial_conf[MAX_VOIP_DIAL_NUM];	//最多2组拨号
};
...
unsigned short comm_param_getAppVersion();
int comm_param_syncTime();
void comm_update_wireless();
void comm_refresh_network();
void comm_refresh_wifi();
void reset_server_config();
//for webrtc
void comm_param_getSystemInitset(comm_system_initset *para);
void comm_param_setSystemInitset(comm_system_initset *para);
void comm_param_stopParamService(void);
