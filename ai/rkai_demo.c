...
int rmm_process_rgb(void *buff, int w, int h)
{
    ...
    // IvaImage.info.format = ROCKIVA_IMAGE_FORMAT_RGB888; //ROCKIVA_IMAGE_FORMAT_YUV420SP_NV12;
    // IvaImage.dataPhyAddr = NULL;
    // IvaImage.dataAddr = buff; // has error
    // IvaImage.dataFd = 0;
    // IvaImage.frameId++;
...
void rmm_yuv_handle_init()
{
	...
	// RKNNYOLO26Detection yolo26 = RKNNYOLO26Detection(0.25);
	// int ret = yolo26.loadModel("/userdata/model/yolo26n_u8_rv1106.rknn");
	// printf("Model load return: %d\n", ret);
    
    // image_t image;
    // image.width = 640;
    // image.height = 640;
    // image.data = malloc(640*640*3);

    // std::vector<RKNNResult> results;
    // yolo26.process(image, results);
