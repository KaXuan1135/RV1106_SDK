...
void app_update_local_info(local_info_t *local_info_p)
{
	...
	memcpy(local_info_p->serial_info,temp.device_serial_no,MAX_COMM_SERIAL_LEN);
					...
					if(iLen == sizeof(isp_config))
					{
						...
						switch(isp_type)
						{
							...
							case ISP_TYPE_DAYNIGHT_CONFIG:
							{
								...
								isp_config.isp_config.dayNight.iMode = htonl(tmp.dayNight.iMode);
								...
								isp_config.isp_config.dayNight.iSens = htonl(tmp.dayNight.iSens);							
								break;
							}
					...
					if(iLen == sizeof(sys_ctrl))
					{
						...
						switch(ctrl_type)
						{
							...
							case SYS_CTRL_TYPE_INFO:
							{
								...
								memcpy(ctrl_info.device_serial_no,version_info.device_serial_no,32);
