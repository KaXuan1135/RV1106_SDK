...
// #if 0	
	...
	// 	comm_serial_info_t si;
	// 	comm_param_getSerialStruct(0,&si);
	// 	os_dbghint("ttys1 baud=%d\n", si.baut_rate);
	// 	if(si.baut_rate != 115200)
	// 	{
	// 		si.baut_rate = 115200;
	// 		comm_param_setSerialStruct(0,si);
	// 	}	
	// }
// #endif	
//	comm_app_register_serial_callback(1,comm_app_serial_data_process,(void*)CUSTOM_NAME_ID); //云视宝单片机通讯
...
static void *IrcutProcessThread(void *pArg)
{
	...
	comm_param_getSysConfig(&sys_config);	
	os_dbg("init ......");
	usleep(1000*1000);	//防止ircut isp没有启动
